...
root.exe -b -q 'Plot_searchBin_exp.C("stacked","searchH_b","Elog410_","Elog409_")'
root.exe -b -q 'Plot_searchBin_exp.C("stacked","QCD_Low",  "Elog410_","Elog409_")'
root.exe -b -q 'Plot_searchBin_exp.C("stacked","QCD_Up",   "Elog410_","Elog409_")'
...
void Plot_searchBin_pre(string sample="stacked",string histname="searchH_b",string elogA="Elog410_",string elogB="Elog410_",
			 int pull=0){
...
//float ymin_top = 0.015;
  ...
  //
  // Prediction 1
  //if(sample.find("stack")==string::npos)sprintf(tempname,"TauHad/%sGenInfo_HadTauEstimation_%s.root",elogA.c_str(),sample.c_str());
  //else sprintf(tempname,"TauHad/Stack/%sGenInfo_HadTauEstimation_%s.root",elogA.c_str(),sample.c_str());  
  if(sample.find("stack")==string::npos)sprintf(tempname,"TauHad2/%sHadTauEstimation_%s.root",elogA.c_str(),sample.c_str());
  else sprintf(tempname,"TauHad2/Stack/%sHadTauEstimation_%s.root",elogA.c_str(),sample.c_str());
  TFile * GenFile = new TFile(tempname,"R");
  printf("Opened %s\n",tempname);

  //
  // Prediction 2
  //if(sample.find("stack")==string::npos)sprintf(tempname,"TauHad/%sGenInfo_HadTauEstimation_%s.root",elogB.c_str(),sample.c_str());
  //else sprintf(tempname,"TauHad/Stack/%sGenInfo_HadTauEstimation_%s.root",elogB.c_str(),sample.c_str());
  if(sample.find("stack")==string::npos)sprintf(tempname,"TauHad2/%sHadTauEstimation_%s.root",elogB.c_str(),sample.c_str());
  else sprintf(tempname,"TauHad2/Stack/%sHadTauEstimation_%s.root",elogB.c_str(),sample.c_str());
  ...
  sprintf(tempname,"VersionA: %s",elogA.c_str());
  catLeg1->AddEntry(GenHist,tempname,"p");
  //sprintf(tempname,"Prediction from MC");
  sprintf(tempname,"VersionB: %s",elogB.c_str());
  ...
  // ----------
	...
	One_NoError->SetBinError(ibin,0.);
      ...
      sprintf(ytitlename,"#frac{Version A}{Version B} ");
...
*/
  ...
  sprintf(tempname,"PlotPre_%s_%s_Full_%s%sPlot.png",histname.c_str(),sample.c_str(),elogA.c_str(),elogB.c_str());
  if (pull==1) 
    sprintf(tempname,"PlotPrePull_%s_%s_Full_%s%sPlot.png",histname.c_str(),sample.c_str(),elogA.c_str(),elogB.c_str());
  canvas->Print(tempname);

  sprintf(tempname,"PlotPre_%s_%s_Full_%s%sPlot.pdf",histname.c_str(),sample.c_str(),elogA.c_str(),elogB.c_str());
  if (pull==1)
    sprintf(tempname,"PlotPrePull_%s_%s_Full_%s%sPlot.pdf",histname.c_str(),sample.c_str(),elogA.c_str(),elogB.c_str());
  canvas->Print(tempname);
