...
//  TFile *file_08TeV         = new TFile("tauTemplatesRes_8TeV.root","R"); 
  ...
  for(int i=0;i<4;i++){
    sprintf(tempname,"hTauResp_%d",i);
    thist_km = (TH1D*)file_13TeV_Koushik->Get(tempname)->Clone();
    ...
    if(i==0)sprintf(tempname,"20 - 30: KM");
    if(i==1)sprintf(tempname,"30 - 50: KM");
    if(i==2)sprintf(tempname,"50 - 100: KM");
    if(i==3)sprintf(tempname,">100: KM");
