...
void  plot_PredictionRatio_forICHEP2016(string histname="searchH_b",string elog="ARElog32",string variable="MinMuPt",string numerator="MinMuPt24",string denominator="MinMuPt20"){  
 ...
 int W = 1200;
  ...
  sprintf(tempNum,"TauHad2/Stack/%s_HadTauEstimation_stacked_%s.root",elog.c_str(),numerator.c_str());
  TFile *file   = new TFile(tempNum,"R");
  printf("Opened %s\n",tempNum);

  sprintf(tempDen,"TauHad2/Stack/%s_HadTauEstimation_stacked_%s.root",elog.c_str(),denominator.c_str());
  TFile *file2   = new TFile(tempDen,"R");
  printf("Opened %s\n",tempDen);


  sprintf(tempRatio,"TauHad2/Stack/%s_Ratio_HadTauEstimation_stacked_%s.root",elog.c_str(),variable.c_str());
