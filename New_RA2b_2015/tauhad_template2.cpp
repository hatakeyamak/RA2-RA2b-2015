...
int main(int argc, char *argv[]){
  ...
  //*AR,Jul29,2016-Constructs an ifstream object, initially associated with the file identified by its first argument
  ...
  Double_t mht_bins[13] = {0., 50.,100.,150.,200.,250.,300.,350.,400.,500.,
    ...
    //*AR, Aug24,2016-This vector of 1D hists is introduced to check whether dphi distribution between reco muon and simulated tau jet changes significantly for different muon pt ranges 
    std::vector<TH1*> htau_SimJetPhi(4);
    for(unsigned int i = 0; i < 4; ++i){
      htau_SimJetPhi.at(i) = new TH1D(utils2::dPhi_name(i),"DPhi between reco muon and simulated tau jet",100,-1.0,1.0);
    }

  //*AR, Aug23,2016- This histogram was added to check how different dphi between reco muon and simulated tau jet from dphi between gen tau and matching reco tau jet obtained in tauhad_template.cpp
  TH2D * tau_SimJetPhi = new TH2D("tau_SimJetPhi","DPhi between muon and simulated tau jet vs. their energy ratio",50.,0.,2.5,100.,-1.0,1.0);
    
    //*AR, Aug24,2016- 1D template for phi of simulated tau jets with respect to reco muon
  TH1D * OneD_tau_SimJetPhi = new TH1D("OneD_tau_SimJetPhi","DPhi between reco muon and simulated tau jet",100.,-1.0,1.0);

  //build a vector of histograms
  TH1D weight_hist = TH1D("weight", "Weight Distribution", 5,0,5);
  vec.push_back(weight_hist);
  //*AR-Jul29,2016-StatError_hist not in tauhad_template.cpp
  ...
  //*AR,Jul29,2016- What is difference between HT and HT2 etc?
  TH1D RA2HT2_hist = TH1D("HT2","HT2 Distribution",14,ht_bins);
  RA2HT2_hist.Sumw2();
  vec.push_back(RA2HT2_hist);
  //*AR-Jul29,2016-RA2origHT_hist not in tauhad_template.cpp
  ...
  //*AR-Jul29,2016-RA2origMHT_hist not in tauhad_template.cpp
  ...
  //*AR-Jul29,2016-tauhad_template.cpp has TauJet distribution instead of Muon distributions
  ...
  //*AR, Jul29,2016- Only for fastsim
  if(subSampleKey.find("fast")!=string::npos){
    ...
    //
  }//End of only fastsim-if(subSampleKey.find("fast")!=string::npos)



    

  //*AR, Jul29,2016-Reads input ntuples from given file list and creates a vector
  while(fin.getline(filenames, 500) ){filesVec.push_back(filenames);}
  cout<< "\nProcessing " << subSampleKey << " ... " << endl;

  //std::cout<<" filesVec.size() "<<filesVec.size()<<endl;    

  for(unsigned int in=0; in<filesVec.size(); in++){ 
    sample_AUX->Add(filesVec.at(in).c_str()); 
  
    //std::cout<<" filename "<< filesVec.at(in).c_str()<<std::endl;
  }
  ...
  //* AR, Jul28,2016 -vec is vector of histograms of distributions like ht, mht, jet, bjet,pt and phi of tau etc. Here for every cut we define this vector of histograms.cut_histvec_map is a map<string , vector<TH1D> > . cut_histvec_map_search is not in tauhad_template.cpp
  ...
  //*AR, Jul30,2016-StudyErrorPropag is true only for getting systematics
  if(StudyErrorPropag){
  ...
  //*AR,Aug6,2016-Note that in AN we mention that The muon reconstruction and identification efficiency, and the muon isolation efficiency, are the same as used for the lost-lepton background determination. So we use them directly from Simon.
  ...
  //*AR, Jul29,2016- What are these IsoEffFile2? Why the same file we are using all the time?
  ...
  //* AR-Jul29,2016- I think hAll_mu and hNonW_mu are equivalent to hW_mu and hTau_mu in tauhad_template.cpp 
  ...
  //*AR, Jul30,2016-TauResponse_nBins=4 as we have four templates based on pt 20-30,30-50,50-100,>100 GeV
  ...
  for(int i=0; i<TauResponse_nBins; i++){
    ...
    //*AR, Jul30,2016- This if loop runs when we run the code for getting template systematics based on JEC uncertainties
    ...
    //*AR, Jul30,2016- Note here that we are just considering 2D template, where x axis is (projection of visible part pt of matched reco tau jet on gen tau pt/gen tau pt) and y axis is (projection of visible part pt of matched reco tau jet normal to gen tau pt/gen tau pt)
 
    sprintf(histname,"hTauResp_%d_xy",i);
    vec_resp_xy.push_back( (TH2D*) resp_file->Get( histname )->Clone() );

  }
  //* AR, Aug1,2016--tau_GenJetPhi is a 2D histogram-(x,y)=(tauJetPt / genTauPt,genTauPhi - tauJetPhi).
  ...
  //*AR,Jul30,2016- This correction will be used for low HT region where single muon trigger uses pt>24 GeV instead of search region pt>20 GeV 
  ...
  //std::cout<<" TauHadModel "<<TauHadModel<<endl;
  // bootstrap or not ?
  int nLoops;  
  int nBtagsForHadTau;

  //*AR, Jul30,2016- In default case bootstrap=ON 
  if(utils2::bootstrap){nLoops=50;nBtagsForHadTau=2;}
  //*AR, Aug8,2016-"else" condition runs when we turn bootstrapoff for getting Tau to Mu probability 
  ...
  while( evt->loadNext() ){
    ...
    //*AR, Jul30, 2016- Some conditions on TTbar sample
    if(!evt->DataBool_()){
	...
	//	  std::cout<<" evt->gen_ht() "<< evt->gen_ht()<<endl;
	...
	*/
    ...
    //*AR,Jul30,2016-These two if conditions wont run for regular prediction
    ...
    //*AR,Jul30,2016-- UncerLoop_size=1

    for(int iuncer=0; iuncer<UncerLoop.size() ;iuncer++){
      //std::cout<<" Ele_UncerLoop "<<UncerLoop[iuncer]<<endl;
      /////////////////////////////////////////////////////////////////////////////////////
	...
	//*AR, Jul30,2016-Runs over a list of all reco muons
	if(evt->MuPtVec_().size()>0)
	  //    std::cout<<"Reco_Muon_size "<<evt->MuPtVec_().size()<<endl;
	    ...
	    //std::cout<<" activity "<<activity_<<endl;

	    //mu_mt_w = sqrt(  2* ( met*objPt -( met*cos(metphi)*objPt*cos(objPhi) + met*sin(metphi)*objPt*sin(objPhi) ) ) );
            double mu_mt_w =utils->calcMT(pt,phi,evt->met(),evt->metphi());

	    //*AR, Jul30,2016-Following two if conditions do not run for regular prediction  
	    ...
	    //*AR-Jul30,2016- Here vec_recoMuon4vec,vec_MTActivity are filled if reco muon satisfies conditions pt>20GeV,eta<2.1,mt<100.vec_recoMuMTW is filled even if mu_mt_w < 100 condition is not satisfied.
            if( pt> LeptonAcceptance::muonPtMin()  && fabs(eta)< LeptonAcceptance::muonEtaMax()  ){
              if(verbose==2)printf(" \n Muons: \n pt: %g eta: %g phi: %g \n ",pt,eta,phi);
              temp4vec.SetPtEtaPhiE(pt,eta,phi,energy);
	      //*AR,Jul30,2016-In default case utils2::applyMT=TRUE. Also for "afterMT" section of calculating efficiency applyMT is true. So vec_recoMuon4vec is filled by muons with pt>20GeV,eta<2.1 and mt<100 
              if(utils2::applyMT){
                if(mu_mt_w < 100. ){vec_recoMuon4vec.push_back(temp4vec);vec_MTActivity.push_back(activity_);}
              }// end of applyMT
	      //*AR,Aug10,2016-"else" condition executes during "beforeMT" section of calculating MT efficiency. So vec_recoMuon4vec is filled by muons with pt>20GeV,eta<2.1 without MT constraint
      ...
      //*AR-Jul30,2016--"else" condition does not run as TauHadModel=4
	...
	for(int i=0; i< evt->GenMuPtVec_().size(); i++){ // Ahmad33
	 ...
	 Ahmad33 */
      ...
      //*AR-Jul30,2016--For us, TauHadModel=4
	...
	for(int i=0; i< evt->GenElecPtVec_().size(); i++){// Ahmad33
      ...
      //	std::cout<<" vec_recoMuMTW.size "<<vec_recoMuMTW.size()<<endl;

      if(vec_recoMuMTW.size()>0)muMtW = vec_recoMuMTW[0]; 
      muMtWHist->Fill(muMtW,eventWeight);
      //	std::cout<<" muMtW "<<muMtW<<endl;
      if(verbose==1)printf(" \n **************************************** \n #Muons: %d #Electrons: %d \n ****************************** \n ",vec_recoMuon4vec.size(),vec_recoElec3vec.size());

      //if( template_nMuons == 1 && template_nElectrons == 0 ) {
      bool pass1=false;

      //*AR, Jul30,2016--Note that vec_recoMuon4vec is filled if reco muon satisfies conditions pt>20GeV,eta<2.1,mt<100(in default case) and without MT constraint for "beforeMT" section of calculating MT.So pass1 becomes true if there is only one muon found satisfying all 3 cuts in default case while satisfying first 2 cuts in "beforeMT" case.
      ...
      //*AR, Jul30,2016-Only if reco muon is found with condition pt>20GeV,eta<2.1,mt<100
	...
	const unsigned int ptBin = utils2::TauResponse_ptBin(muPt);
	...
	//*AR,Jul30,2016-If there are more than one electron or muon found in event, then skip the event. So we have only the events with muon satisfying pt, eta, mt conditions
	...
	GenRecMu_all++;
	//*AR_Jul30,2016- If reco muon does not match a GenMuon, GenRecMu_fail increases 
	int GenMuIdx=-1;
	...
	//*AR-nLoops=50 for bootstrap=true and nLoops=1 for bootstrap=false(getting tau->mu probability)
	  ...
	  //*AR,Jul30,2016-By default bootstrap is on
	  if(utils2::bootstrap){
	    //*AR,Jul30,2016--Here based on reco muon pt, a template is chosen. Then value of bin center of l'th bin is returned as scale.
	    scale = utils->GetBinValue(muPt,vec_resp,l );

	    //*AR,Jul30,2016--This part does not run for ordinary prediction
	    if(subSampleKey.find("template")!=string::npos){
	  ...
	  //*AR,Jul30,2016-"else" condition does not run for default case 
	  //*AR, Aug8,2016-"else" condition runs when we turn bootstrap off for getting Tau to Mu probability
	  //*AR, Aug8,2016-a random value(bin) is chosen from a template selected based on muPt 
	  else scale = utils->getRandom(muPt,vec_resp );
	  Double_t scale_x=0,scale_y=0;
  
	  //*AR,Jul30,2016-scale_x,scale_y will be randomly chosen from a 2D template
	  //*AR,Aug8,2016-Note that scale_x,scale_y defined here based on 2D template and used to get simTauJetPhi_xy/simTauJetPt_xy are used only in case of bootstrap=false(getting tau->mu probability).Because simTauJetPhi_xy/simTauJetPt_xy are redefined later inside if(utils2::bootstrap) condition which runs in default case. 
	  utils->getRandom2(muPt,vec_resp_xy,scale_x,scale_y );
	  //*AR-Jul30,2016-Here we simulate a tau jet by smearing a reco muon jet pt by 1D tau response template and eta and phi same as muon jet 
	  simTauJetPt = scale * muPt;
	  simTauJetEta = muEta;
	  simTauJetPhi = muPhi;

	  //*AR,Jul30,2016-simTauJetPt_x/simTauJetPt_y are two components obtained after smearing x and y components separately.
	  double simTauJetPt_x = scale_x * muPt;  
	  double simTauJetPt_y = scale_y * muPt;


	  //*AR,Jul30,2016-As x component,simTauJetPt_x represents the component along muon jet pt, phi of a simulated tau jet will be muPhi plus deltaPhi calculated from x, y components.
	  ...
	  //*AR,Jul30,2016-scale is the bin centre for l'th bin.scale is divided by binwidth to find which bin "scale" corresponds to.So binx=l-1.
	  //*AR,Aug1,2016-binx=l-1 not causing any issue?
	  ...
	  //	    std::cout<<" l "<< l<< " scale "<<scale<< " binx "<<binx<<endl;

	  //*AR-By default, bootstrap="ON"
	  if(utils2::bootstrap){
	    double phi_genTau_tauJet=0.;
	    if(binx!=0){
	      //* AR, Aug1,2016--tau_GenJetPhi is a 2D histogram-(x,y)=(tauJetPt / genTauPt,genTauPhi - tauJetPhi).
	      //*AR, Aug5,2016--Here dPhi=genTauPhi - tauJetPhi is chosen randomly
	      if(verbose!=0)cout << "deltaPhi: " << h2tau_phi->ProjectionY("angularTemplate",binx,binx,"")->GetRandom() << endl;

	      //*AR,Aug1,2016-Here why we use binx instead of using l?
	      phi_genTau_tauJet=h2tau_phi->ProjectionY("angularTemplate",binx,binx,"")->GetRandom();    
	      //		if(binx==0)
	      //		  std::cout<<" eventN "<<eventN<<" l "<<l<<" binx "<<binx<<" ProjectionY_binx "<<phi_genTau_tauJet<<endl;
	    }
	    simTauJetPhi_xy=simTauJetPhi + phi_genTau_tauJet ;
	    double AR_simTauJetPhi_xy=simTauJetPhi - phi_genTau_tauJet ;
	    simTauJetPt_xy=simTauJetPt;

	    if(TVector2::Phi_mpi_pi(muPhi - AR_simTauJetPhi_xy) < -1.0){
	      htau_SimJetPhi.at(ptBin)->Fill(-1.0 ,eventWeight);
	      tau_SimJetPhi->Fill(scale ,-1.0 ,eventWeight);
	      OneD_tau_SimJetPhi->Fill(-1.0 ,eventWeight);
	    }
	    else if(TVector2::Phi_mpi_pi(muPhi - AR_simTauJetPhi_xy) > 1.0){
	      htau_SimJetPhi.at(ptBin)->Fill(1.0 ,eventWeight);
	      tau_SimJetPhi->Fill(scale , 1.0 ,eventWeight);
	      OneD_tau_SimJetPhi->Fill(1.0 ,eventWeight);
	    } 
	    else{
	      htau_SimJetPhi.at(ptBin)->Fill(TVector2::Phi_mpi_pi(muPhi - AR_simTauJetPhi_xy) ,eventWeight);
	      tau_SimJetPhi->Fill(scale , TVector2::Phi_mpi_pi(muPhi - AR_simTauJetPhi_xy) ,eventWeight);
	      OneD_tau_SimJetPhi->Fill(TVector2::Phi_mpi_pi(muPhi - AR_simTauJetPhi_xy) ,eventWeight);
	    }
 	  }
	  ...
	  //*AR, Aug1,2016-simTauJetEta=muEta. Now we have 3 vector of original muon and also simulated tau jet.
	  //*AR, Aug8,2016-
	  ...
	  //*AR,Aug1,2016-Looks for a matcing reco jet to a single muon found with muon pt>20GeV,eta<2.1,mt<100
	  ...
	  //*AR,Aug1,2016-Just to check if there is no matching jet found
	  if (slimJetIdx==-1){
	  ...
	  if(slimJetIdx==-1){
	    ...
	    //*AR,Aug1,2016-If there is no slimJet found matching a reco muon, then simulated tau jet(SimTauJet) is added as first element in HT3JetVec,MHT3JetVec if it satisfies pt, eta conditions.
	    ...
	    //	      std::cout<<" HT3JetVec_size "<<HT3JetVec.size()<<" MHT3JetVec_size "<<MHT3JetVec.size()<<endl;
	  }  //end of if(slimJetIdx==-1)

	  //* AR-Aug5,2016-This cout for example returns following numbers. I do not understand why nJets do not match any of the other two numbers
	  //nJets 4 jets 11 slimJets 18, nJets 5 jets 13 slimJets 19
	  //	    std::cout<<" nJets "<<evt->nJets()<<" jets "<<evt->JetsPtVec_().size()<<" slimJets "<<evt->slimJetPtVec_().size()<<endl;

	  for(int i=0;i<evt->slimJetPtVec_().size();i++){
	    //*AR,Aug1,2016-If there is matching slimJet found, following "if" condition will run over remaining slimjets, while if no matching found, it will run over all slimjets.
	    //*AR,Aug1,2016-So HT3JetVec/MHT3JetVec have only slimjets satisfying pt>30,eta<2.4/5 for HT3/MHT3
	    //*AR,Aug1,2016-Why HT3JetVec/MHT3JetVec use different eta cuts
	    if(i!=slimJetIdx){
	      ...
	      double jecCorr = evt->slimJetjecFactor_()[slimJetIdx];
	      //		std::cout<<" jecCorr "<<jecCorr<<endl;
	      if (jecCorr==0.) jecCorr=1.;
	      //*AR,Aug1,2016-Here first multiplication gives matching slimjet 3 vector if there were no issues with jet energy measurement.Then we subtract muon 3 vector as muon measurement does not involve jet measurement issues.Then remaing fraction we multiply by reciprocal of jecCorr to get back to reco measured jet energy and then add SimTauJet3Vec to replace muon reco energy.
	  ...
	  //*AR,Aug5,2016-if(slimJetIdx!=-1) there are three cases possible-
	  //HT3JetVec.size()= evt->nJets() (mostly observed)
	  //HT3JetVec.size()=evt->nJets()-1 (in case matched slimjet was originally hard jet but after smearing it fails kinematic,pt>30,eta<2.4 cut)
	  //HT3JetVec.size()=evt->nJets()+1 (in case matched slimjet was originally soft jet but after smearing it satisfies kinematic,pt>30,eta<2.4 cut)

	  //*AR,Aug5,2016-In 10000 events not a single event is found with slimJetIdx==-1


	  //  std::cout<<" event "<<eventN<<" l "<<l<<" slimJetIdx "<<slimJetIdx<<" nJets "<<evt->nJets()<<" HT3JetVec "<<HT3JetVec.size()<<" MHT3JetVec "<<MHT3JetVec.size()<<endl;
	  //std::cout<<" Before smearing "<<" pt "<<evt->slimJetPtVec_()[slimJetIdx]<<" eta "<<evt->slimJetEtaVec_()[slimJetIdx]<<" After smearing "<<" pt "<<NewTauJetPt<<" eta "<<NewTauJetEta<<endl;
	  ...
	  } //End of "fastsim" with genHTMHT


	    //*AR,Aug1,2016-HT3JetVec,MHT3JetVec are ready to use with jetVecs satisfying pt/eta conditions
            // Order the HT3JetVec and MHT3JetVec based on their pT(descending order)
	  ...
	  //*AR,Aug1,2016-New HT,MHT are determined for the event after replacing muon pt(3 vec) by SimTauPt(3 vec)
	  ...
	  // *AR- only for fastsim with genHTMHT version	    
	  ...
	  //*AR,Aug1,2016-By default, bootstrap is true. 
	  //*AR,Aug8,2016-bootstrap is false while getting tau to mu probability. So here we are again finding a bmistag rate for matching jets.
	  if(!utils2::bootstrap){
	    JetIdx=-1;
	  ...
	  //*AR, Aug1, 2016- In default case nBtagsForHadTau=2 as there are two cases possible HadTau is b mistagged or not.
	  //*AR, Aug8, 2016-When bootstrap=false(calculating tau->mu probability)nBtagsForHadTau =1 so loop will execute only once for m==0  
	  for(int m=0; m<nBtagsForHadTau;m++){
	    ...
	    //*AR,Aug5,2016-NewTauJetPt is pt of matched slimjet( to muon) after replacing muon pt by simTauJetPt  
	    ...
	    //AR,Aug8,2016-This loop won't execute while getting tau->mu probability as bootstrap=false then 
	    if(utils2::bootstrap){
	      double bRate_Plus = bRate + bRate * .5;
	      double bRate_Minus = bRate - bRate * .5;
	      //		std::cout<<" m "<<m<<endl;
	      if (m==0){
		//*AR, Aug1,2016-Probability that tau is not mistagged is (1.-bRate)
		...
		//*AR, Aug5,2016-NewNB is increased by 1 in case of probability that taujet is mistagged
		NewNB++;
		//*AR, Aug1,2016-Probability that tau is mistagged is bRate
	    ...
	    //*AR,Aug1,2016-"else" condition not relevent in default case but runs while getting tau->mu rate with bootstrap =false
	    else {
	      // get a random number between 0 and 1
	      //*AR,Aug8,2016-So here we select a random number from (0,1). If bRate corresponding to simTauJet pt is higher than this random number, then nbjets increased by one
	    ...
	    //New MET
	    //*AR,Aug1,2016-As met is negative of vector sum of pt, first we have to add muPt component from met component and then subtract simTauJetPt componet to get new metX /metY after smearing.
	    ...
	    double newMetphi=-99.;
	    //*AR,Aug5,2016-New MET direction(phi) after replacing mupt by simTauJetpt
	    ...
	    // New minDelPhi_N
	    //*AR,Aug1,2016-Not understood DeltaT
	    double dpnhat[3]; 
	    ...
	    //*AR,Aug5,2016-It is a usual case that evt->JetsPtVec_()<evt->slimJetsPtVec_() as there is a jet pt cut of about 10 GeV or so on Jets. But it is found that slimJetIdx=JetIdx as in slimJets we are filling Jets before softJets so matching index remains same in both vectors.
 
	    utils->findMatchedObject(JetIdx,muEta,muPhi,evt->JetsPtVec_(),evt->JetsEtaVec_(), evt->JetsPhiVec_(),deltaRMax,verbose);  

	    //*AR,Aug5,2016-I do not find this reassignment of NewNB convincing. As mentioned before when there is a matching jet found 3 cases are possible-HT3JetVec.size()>,= or < evt->nJets(). So i expect 3 if statements to be there.
	    //*AR,Aug6,2016-Note that evt->nBtags() returns a number satisfying conditions- evt->JetsPtVec_()[j]>30 && fabs(evt->JetsEtaVec_()[j])<2.4 && evt->csvVec()[j]>evt->csv_().
	    //*AR,Aug6,2016-HT3JetVec.size() > (int) evt->nJets() and JetIdx!=-1 and evt->csvVec()[JetIdx]>evt->csv_()then NewNB=evt->nBtags()+1
	    //*AR,Aug6,2016-else if HT3JetVec.size() < (int) evt->nJets() and JetIdx!=-1 and evt->csvVec()[JetIdx]>evt->csv_()then NewNB=evt->nBtags()-1
	    //*AR,Aug6,2016-else if HT3JetVec.size() = (int) evt->nJets() and JetIdx!=-1 then NewNB=evt->nBtags()
	    //*AR,Aug6,2016-else if JetIdx==-1 then NewNB=evt->nBtags()
	    ...
	    /*	      if( (int) HT3JetVec.size() == (int) evt->nJets() && evt->csvVec()[JetIdx]<evt->csv_() ){
		      std::cout<<" event "<<eventN<<" l "<<l<<" slimJetIdx "<<slimJetIdx<<" nJets "<<evt->nJets()<<" HT3JetVec "<<HT3JetVec.size()<<" MHT3JetVec "<<MHT3JetVec.size()<<endl;
		      std::cout<<" Before smearing "<<" pt "<<evt->slimJetPtVec_()[slimJetIdx]<<" eta "<<evt->slimJetEtaVec_()[slimJetIdx]<<" After smearing "<<" pt "<<NewTauJetPt<<" eta "<<NewTauJetEta<<endl;
		      std::cout<<" jets "<<evt->nJets()<<" btags "<<evt->nBtags()<<" matchedJetidx "<<JetIdx<<endl;
		      }    */

	    // New dphi1, dphi2, and dphi3
	    double newDphi1=-99.,newDphi2=-99.,newDphi3=-99.,newDphi4=-99.;
	    //first order the jets based on their pT
	    //*AR Aug6,2016-I see that HT3JetVec is already ordered at this point. i don't know why it is done again here.
	    ...
	    //*AR,Aug6,2016-deltaPhi for first four leading jets is calculated with respect to new MHT direction after smearing reco muon/matching reco jet
	    ...
	    //*AR,Aug6,2016-Following two variables are only used in eveinfvec
	    ...
	    //*AR-Aug6,2016-METtrigEffCorr=1 if ApplyMETEff=false(MC)
		...
		if (muPt<LeptonAcceptance::muonPtMinLowHT()) trigEffCorr=0.;
		  ...
		  //*AR, Aug6,2016-Here trigEffCorr is set for LowHT
		...
		MuonPtMinCorr=1./MuonPtMinSelectionEfficiency;
	      ...
	      //*AR, Aug6,2016-Here trigEffCorr is set for HighHT
	      else {
	    ...
	    //*AR,Aug6,2016-Note that in AN we mention that The muon reconstruction and identification efficiency, and the muon isolation efficiency, are the same as used for the lost-lepton background determination parameterized in pt and activity. So we use them directly from Simon.
	    //*AR,Aug6,2016-Eff_Arne multiplies reconstruction and isolation efficiencies, but error in two cases we are defining separately as Reco_error_Arne and Iso_error_Arne
	    ...
	    }//if(evt->DataBool_()) ends here

	    // *AR,Aug6,2016-if baseline cuts on the main variables are passed then calculate the acceptance otherwise simply take 0.9 as the acceptance for both high and low deltaPhi.
	    ...
	    //*AR,Aug6,2016-We are parameterizing acceptance in 6 njet bins(3,4,5,6,7-8,>=9)*12HT-MHT bins(QCD bins with highest two MHT bins combined
	    ...
	    //*AR,Aug6,2016-Precaution: In case Acc/Acc_lowDphi/Eff/Eff_Arne are zero, assign them default values, as they go in the denominator of weight calculation. 
	    //*AR,Aug6,2016-Still to investigate where Eff is used?
	    if(Acc==0){Acc=0.9;cout << " Warning! Acc==0 \n ";}
	    if(Acc_lowDphi==0)Acc_lowDphi=0.9;
	    if(Eff==0)Eff=0.75;
	    if(Eff_Arne==0){Eff_Arne=0.75;cout << " Warning! Eff_Arne==0 \n ";}

	    //*AR,Aug6,2016-Note that there is no default value set for AccError/Acc_lowDphiError in case baseline selection is not satisfied. In that case values returned are AccEr 2.122e-314 Acc_lowDphiEr 2.08421e-317 etc.
	    ...
	    //*AR,Aug6,2016-Only runs while calculating systematics
	    if(StudyErrorPropag){
	    ...
	    } //end of if(StudyErrorPropag)

	      //*AR,Aug6,2016-Remember Eff_Arne was product of reconstuction and isolation efficiencies obtained from Simon
	    ...
	    //*AR,Aug6,2016- For us TauHadModel=4 
	    ...
	    //*AR-Aug6,2016-binNum_ProbTauMu is a temporary variable defined to check if Prob_Tau_mu=0 and then assign default value 0.1 to it and value 0.0 to corresponding error
	    ...
	    //*AR,Aug6,2016-Remember Eff_Arne was product of reconstuction and isolation efficiencies obtained from Simon.
	    //*AR,Aug6,2016-So here if we compare totWeight to AN formula, from efficiency terms remaining ones are trigger,mT,isotrack veto efficiencies, dilepton contamination 
	    ...
	    //*AR,Aug6,2016-We have TauHadModel=4
	    if(TauHadModel>=3){
	    ...
	    //*AR,Aug6,2016-trigger effciency is multiplied here. Only things left are to account for bmistag rate and tau response template and mT,isotrack veto efficiency
	    ...
	    //*AR,Aug9,2016-So here weightEffAcc and totWeight are without bmistag rate and tau response template and mT,isotrack veto efficiency 
	    ...
	    //*AR,Aug8,2016-This loop wont execute to get tau->mu probability.So totWeight is not updated for bootstrapWeight and Prob_Btag
	    if(utils2::bootstrap){
	      //*AR-Aug6,2016-Based on muon pt a template is chosen from vec_resp and bin content of l'th bin is multiplied by bin width to get corresponding weight.
		...
		bootstrapWeight = utils->GetBinContent(muPt,vec_respDown,l) * utils->GetBinWidth(muPt,vec_respDown,l);
	      ...
	      //*AR,Aug6,2016-bmistag rate, tauresponse from template are applied here
	      totWeight*=bootstrapWeight;
	      totWeight*=Prob_Btag;
	      totWeight_lowDphi*=bootstrapWeight;
	      totWeight_lowDphi*=Prob_Btag;
	    } //end of (utils2::bootstrap)
	    ...
	    //*AR,Aug10,2016-mtWeight is read from MtFile in default case , but set to 1 in case utils2::CalcMT=true
	    ...
	    //*AR,Aug6,2016-Assigns default value to mtWeight in case mtWeight=0
	    if(mtWeight==0){mtWeight=0.9;cout << " Warning! mtWeight==0 \n ";}
	    if(mtWeight_lowDphi==0)mtWeight_lowDphi=0.9;
	    //*AR,Aug6,2016-By default,utils2::CalcMT is false
	    if(utils2::CalcMT){
	      //*AR,Aug10,2016-for "beforeMT" case of calculating efficiency,CalcMT=true. So here mtWeight and mtWeight_lowDphi is reset to unity
	      mtWeight = 1.;mtWeightError=0.;
	      mtWeight_lowDphi = 1.;mtWeightError_lowDphi=0.;	
	    }	      
	    //*AR,Aug6,2016-By default,utils2::applyMT is true.Also for "afterMT" section of calculating mt efficiency applyMT=true.But as CalcMT=true in that case mtWeights are set to 1 and mterrors set to 0

	    // Apply MT efficiency
	    //*AR,Aug10,2016-This "if" condition wont execute for "beforeMT" section of calculating efficiency as applyMT=false there. So totWeight wont be updated for mtWeight efficiency in that case 
	    if(utils2::applyMT){
	      ...
	      //*AR,Aug6,2016-By default,utils2::calcMT is false
	      //*AR,Aug6,2016-Here weightEffAcc=totWeight
	      //*AR,Aug6,2016-mT weight is applied here in default case but not while calculating MT efficiency(both for beforeMT and afterMT sections).So in default case remaining is only isotrack veto efficiency
	    ...
	    bool PassIso2=false;
	    //*AR,Aug7,2016-Here total weight for high deltaPhi(search region is called as searchWeight) 
	    //*AR,Aug10,2016-In default case at this point totWeight is updated with all corrections except Isotrkefficiency but for for "beforeMT" case of calculating MT efficiency it is without mtWeight and isotrk efficiency.
	    double searchWeight = totWeight;
	    // applyIsoTrk here 
	    //*AR,Aug7,2016-By default,utils2::applyIsoTrk is true and utils2::IsoTrkModel=0
	    //*AR,Aug10,2016-For "beforeMT" section of calculating efficiency applyIsotrk=false so this loop won't execute
	    ...
	    if(utils2::applyIsoTrk){
	      ...
	      //*AR,Aug7,2016-utils2::IsoTrkModel==1 is not relevant for us
		...
		if(nIsoElec==0&&nIsoMu==0&&nIsoPion==0)PassIso2=true; 
	      } //end of utils2::IsoTrkModel==1

	      int binNum = binMap_ForIso[utils2::findBin_ForIso(newNJet,newHT,newMHT).c_str()];
	      if(utils2::IsoTrkModel==0){
		//*AR,Aug7,2016-hIsoEff is Isolated track veto efficiency. First we set all values for High deltaPhi and then same process will be repeated for Low deltaPhi
		...
		//*AR,Aug7,2016-IsoTrkVetoEff_Nb are the correction factors introduced to cancel observed Nb dependance
		...
		*/
		//*AR,Aug7,2016-Precaution in case IsoTrkWeight is zero.
		//*AR,Aug7,2016-On what basis the default values are set for different efficiencies
		if(IsoTrkWeight==0){
		  IsoTrkWeight=0.6; 
		  //cout << "Warning! IsoTrkWeight==0 \n ";
		  //std::cout << newNJet  << " " << NewNB << " " << newHT << " " << newMHT << std::endl; 
		}
	      } //end of utils2::IsoTrkModel==0
		...
		<< IsoTrkVetoEff_Nb << std::endl;
	      */
	      //*AR,Aug7,2016-Remember that weightEffAcc was same as totWeight for high deltaPhi. Still to understand why then two variables?
	      searchWeight = totWeight*IsoTrkWeight;
	      weightEffAccForEvt = weightEffAcc*IsoTrkWeight;

	    }// end of if applyIsoTrk
	    //*AR, Aug10,2016-"else" condition will execute for "beforeMT" section of calculating efficiency as applyIsoTrk is false there. So in that case searchWeight is without mtWeight and isotrk weight
	    ...
	    //*AR,Aug7,2016-If there is no matching gen muon found to reco muon or if it is found and it's index corresponds to Mu from tau then isMuFromNonW =true 
	    bool isMuFromNonW = false;
	    if(GenMuIdx<0)isMuFromNonW=true;
	    else if(evt->GenMuFromTauVec_()[GenMuIdx]==1)isMuFromNonW=true;
	    //*AR,Aug10,2016-Pass_MuMomForMT variable is defined and used while calculating mt efficiency( to fill searchH histogram) 
	    bool Pass_MuMomForMT = false;
	    //*AR,Aug7,2016-In default case utils2::CalcMT is false so Pass_MuMomForMT is true.If CalcMT=true Pass_MuMomForMT becomes true only if there is a matching gen muon found and it is not coming from tau.
	    if(utils2::CalcMT){
	    ...
	    if(sel->ht_500(newHT) && sel->mht_200(newMHT) && sel->low_dphi(newNJet,newDphi1,newDphi2,newDphi3,newDphi4) && sel->Njet_4(newNJet)  ){
	      //*AR, Aug7,2016-In default case, bootstrap is true, so this loop wont execute
	      //*AR, Aug9,2016-It will execute while bootstrap is off in case of calculating tau->mu probability to update input file TauHad2/Stack/ARElog52_modifiedProbability_Tau_mu_stacked.root
	      
	      if(!utils2::bootstrap){
		// Non W muons calculation
		if(!isData){
		  hAll_mu_lowDelphi->Fill(binMap_ForIso[utils2::findBin_ForIso(newNJet,newHT,newMHT).c_str()],eventWeight);
		  //*AR, Aug9,2016-Filling the histograms that can be used to get tau->mu probability for low dphi. I dont understand why following 2 cases are treated equivalent for getting tau->mu probability
	    ...
	    if(sel->ht_500(newHT) && sel->mht_200(newMHT) && sel->dphi(newNJet,newDphi1,newDphi2,newDphi3,newDphi4) && sel->Njet_4(newNJet) ){
	      //*AR, Aug7,2016-In default case, bootstrap is true, so this loop wont execute
	      if(!utils2::bootstrap){
		//*AR,Aug9,2016-Why isoefficiencies are recalculated here, while we are using the isoefficiencies calculated from tauhad_template.cpp
		...
		// Non W muons calculation
		if(!isData){ 
		  //*AR, Aug9,2016-Filling the histograms that can be used to get tau->mu probability for low dphi
		...
		if(trigPass)trig_pass->Fill(binMap[utils2::findBin_NoB(newNJet,newHT,newMHT).c_str()],eventWeight); 
	      ...
	      //*AR,Aug7,2016-For IsoTrkModel=0 PassIso2 is true
	      if(PassIso2){

		// to see the dileptonic contamination
		//*AR,Aug7,2016-In default case bootstrap is true
		//*AR,Aug9,2016-Filling histograms which can be used to get dilepton contamination. Remember that totWeight is alredy accounting for this correction introduced before. 
		...
		//*AR,Aug7,2016- In default case and also while getting tau->mu probability Pass_MuMomForMT is true
		//*AR,Aug7,2016- searchWeight is the most updated weight after applying the final isotrack veto efficiency in default case(note that for tau->mu probability(bootstrap=false) totWeight(hence searchWeight) is without bootstrap and bmistag weigjhts). So we are ready to fill the histograms now.
		...
		if(fastsim){
		  ...
		  //*AR-Aug7,2016-These HT,MHT,Njet seem to be old
		  ...
		  //*AR,Aug7,2016-As NjNb correction is different for QCD and search bin we have introduced -factor_Up_NjNb=QCD_UpNjNbCorr/NjNbCorr
		...
		//*AR, Aug7,2016-What is the use of these correlation histograms?
	    ...
	    }   // end of loop over sel(baseline cut)


	    //*AR,Aug7,2016- Now similar procedure applied for low delta Phi region
	    // Fill QCD histogram
	    ...
	    if(sel->ht_500(newHT) && sel->mht_200(newMHT) && sel->Njet_4(newNJet) && sel->low_dphi(newNJet,newDphi1,newDphi2,newDphi3,newDphi4)  ){
	      //*AR,Aug7,2016- As totWeight was defined for High deltaPhi region we need to correct it with ratio of efficiencies for Low/High deltaPhi region 
		...
		searchWeight = totWeight/(1-Prob_Tau_mu)*(1-Prob_Tau_mu_lowDelphi)*IsoTrkWeight_lowDphi*mtWeight/mtWeight_lowDphi*Acc/Acc_lowDphi;
	      ...
	      //*AR,Aug10,2016-"else" condition will run for "beforeMT" case of calculating efficiency where applyIsoTrk=false and CalcMT=true. So mtWeight=1 and mtWeight_lowDphi=1 in that case  
	    ...
	    //*AR,Aug7,2016-In default case StudyErrorPropag is false
	    if(StudyErrorPropag){
	      // b mistag error propagation
	      //*AR, Aug9,2016-Remember that when bootstrap is "true", totWeight is updated with Prob_Btag and bootstrap weight while in case of bootstrap="false", totWeight was without Prob_Btag and bootstrap weight. So here totWeightMap is modified for BMistagPlus/Minus when bootstrap is "true" 
	    ...
	    bool pass0=false;
	    //*AR, Aug7,2016-As TauHadModel=4 for us, pass0=true
	    ...
	    if(pass0){
	      ...
	      //*AR, Aug7,2016-Remember map_map was a map of type map<string, map<string , vector<TH1D> > > such that map_map[eventType[i]]=cut_histvec_map;
		...
		//*AR, Aug7,2016-I don't understand why this if "true" here?
		if(true){//we can apply extra selection for each eventType here
		  ...
		  //*AR, Aug7,2016-Remember cut_histvec_map is a map of type map<string , vector<TH1D> > such that cut_histvec_map[sel->cutName()[i]]=vec
		  for(map<string , vector<TH1D> >::iterator ite=cut_histvec_map.begin(); ite!=cut_histvec_map.end();ite++){
		    ...
		    */
		    // *AR,Aug7,2016-This if loop is especially because we want to apply IsoTrkVeto after below mentioned cuts are done(PreSel, nolep, Njet_4, ht_500 and mht_200)
		    if(ite->first!="PreSel" && ite->first!="nolep"&&ite->first!="ht_500"&&ite->first!="mht_200"&&ite->first!="Njet_4" 
		      ...
		      //*AR,Aug7,2016-Here itt->first gives eventtype and totWeightMap[itt->first] returns totWeight 
		      //*AR,Aug7,2016-Remember totWeight was last modified after applying mTweight, so it does not include isotack veto eff.
		    ...
		    //*AR,Aug10,2016-"else" condition runs for "beforeMT" section of calculating mT efficiency where applyIsoTrk is "false". Remember that in that case totWeight is without isotrack efficiency and mtWeight(mtWeight=1)
		    ...
		    } //end of if(sel->checkcut_HadTau.......

		  }//end of loop over cut names (cut_histvec_map)

		  ////EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts//EndOfCuts
                    
		}//end of if "true"
	      }//end of loop over event types(map_map_evt)
	    } //end of if(pass0)
	      //*AR, Aug7,2016-Why following 3 closing loops say "mismatched parenthesis"
	  } //end of loop over m(nBtagsForHadTau)Btag bootstrap
	} // end of loop over l(50 bins of bootstrapped template)

      } // End of if(pass1) which is true only if there is exactly one reco muon with pt>20, eta<2.4

        //*AR, Aug7,2016-Why these new histograms for studying weights?
        // Studying weights
	...
	if (searchH_b_evt->GetBinContent(ibin)>0.){
      ...
      //*AR, Aug7,2016-correlation studies?
  ...
  MuJetfile.Close();
	...
	//*AR, Aug24, 2016-For dPhi distribution between reco muon and simulated tau jet
    for(unsigned int i = 0; i< htau_SimJetPhi.size(); ++i) {
      if(  htau_SimJetPhi.at(i)->Integral("width") > 0. ) {
        // if option "width" is specified, the integral is the sum of the bin contents multiplied by the bin width in x.
         htau_SimJetPhi.at(i)->Scale(1./htau_SimJetPhi.at(i)->Integral("width"));
      }
    }
  if(  OneD_tau_SimJetPhi->Integral("width") > 0. )
    OneD_tau_SimJetPhi->Scale(1./OneD_tau_SimJetPhi->Integral("width"));
  ...
  tau_SimJetPhi->Write();
  OneD_tau_SimJetPhi->Write();
	for(unsigned int i = 0; i < htau_SimJetPhi.size(); ++i) {
	  htau_SimJetPhi.at(i)->Write();
	}
  //KH-Feb2016-starts
  hSearchBinHT->Write();
  hSearchBinMHT->Write();
