...
Plot_TauId_IndivBG(int bgtype=0, string histname="searchH_b_", string cutname="tauId1124_iso", 
  ...
  */
  ...
  TFile * ExpHadTau_TTbar = new TFile("TauHad/GenInfo_HadTauEstimation_filelist_Spring15_TTbar_.root","R");
  TFile * ExpHadTau_WJet = new TFile("TauHad/GenInfo_HadTauEstimation_filelist_Spring15_WJet_.root","R");
  TFile * ExpHadTau_T = new TFile("TauHad/GenInfo_HadTauEstimation_filelist_Spring15_T_.root","R");
  ...
  */
  ...
  TH1D * hExpTT, * hExpWJ, * hExpT, * hExpZJ, * hExpQCD, * hTauHad;
  TH1D * hExpSignal1;
  ...
  sprintf(tempname,"allEvents/%s/%s",cutname.c_str(),histname.c_str());
  hTauHad=(TH1D*)ExpHadTau_TTbar->Get(tempname)->Clone("ExpTauHad");
  hTauHad->Add((TH1D*)ExpHadTau_WJet->Get(tempname)->Clone("ExpTauHadWJet"));
  hTauHad->Add((TH1D*)ExpHadTau_T->Get(tempname)->Clone("ExpTauHadT"));
  // lost lepton is the whole TTbar + WJet + T minus the hadronic tau part of it
  TH1D *  hLostLep = static_cast<TH1D*>(hExpTT->Clone("hLostLep"));
  hLostLep->Add(hExpWJ);
  hLostLep->Add(hExpT);
  hLostLep->Add(hTauHad,-1);
  ...
  hTauHad->SetFillColor(kBlue);
  hLostLep->SetFillColor(kMagenta-1);
  hExpZJ->SetFillColor(kCyan);
  hExpQCD->SetFillColor(kGreen);
  
  TH1D * hExp = static_cast<TH1D*>(hTauHad->Clone("hExp"));
  hExp->Add(hLostLep);
  hExp->Add(hExpZJ);
  hExp->Add(hExpQCD);

  ExpStack->Add(hExpQCD);
  ExpStack->Add(hTauHad);
  ExpStack->Add(hLostLep);
  ExpStack->Add(hExpZJ);
  ...
  */
  if(bgtype==0){
	  ...
	  */
	  sprintf(tempname,"tau had");
	  catLeg1->AddEntry(hTauHad,tempname,"f");
	  sprintf(tempname,"lost lepton");
	  catLeg1->AddEntry(hLostLep,tempname,"f");  
...
*/
  ...
  */
  string type ="all";
  if(bgtype==1)type="tauhad";
  else if(bgtype==2)type="lostlept";
  else if(bgtype==3)type="ZJet";
  else if(bgtype==4)type="QCD";
  
  sprintf(tempname,"tauId_SignalVsBG_indivBG_%s_%s_%s_Plot.pdf",type.c_str(),cutname.c_str(),histname.c_str());
