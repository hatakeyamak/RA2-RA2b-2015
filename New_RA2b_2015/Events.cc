...
bool Events::loadNext() {
  ...
  if( currentEntry_ == template_Entries ) {
      ...
      template_AUX->GetEntry(currentEntry_);
      return true;
    }
}
  ...
  double Events::weight() const { return template_evtWeight; }

  // HT and MHT
  double Events::ht() const { return HT; }
  double Events::mht() const { return template_mht; }
  double Events::mhtPhi() const { return template_mhtphi; }
  // Number of HT jets
  int Events::nJets() const { return cntNJetsPt30Eta24; }
  // DeltaPhi between leading three MHT jets and MHT
  double Events::deltaPhi1() const { return dPhi0; }
  double Events::deltaPhi2() const { return dPhi1; }
  double Events::deltaPhi3() const { return dPhi2; }
  // Generator-level decay information
  vector<int>  *Events::W_emuVec_() const {return W_emuVec;}
  vector<int>  *Events::W_tau_emuVec_() const { return W_tau_emuVec; }
  vector<int>  *Events::W_tau_prongsVec_() const {return W_tau_prongsVec;}
  vector<double>  *Events::muonsMtw_() const {return muonsMtw;}
