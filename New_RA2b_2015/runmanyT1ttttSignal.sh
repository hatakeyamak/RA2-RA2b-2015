...
#for m in 600 ; do
	...
	if [ -e InputFiles_Signal/RA2bin_T1tttt_${m}_${d}_fast ]; then
	    ...
	    if [ $type -eq 0 ]; then
		if [ -e TauHad2Multiple/HadTauEstimation_${filename}-${outStr}_00.root ]; then
		    echo warning !
		    echo exist TauHad2Multiple/HadTauEstimation_${filename}-${outStr}_00.root
		else
		    echo submitting TauHad2Multiple/HadTauEstimation_${filename}-${outStr}_00.root
		    qsub -l walltime=24:00:00 -N ${filename}_${outStr} -o qsub/ -e qsub/ -V $submitscript  
